...
public class ToogleObject : MonoBehaviour
{
    ...
    public void OnClickToogle()
    {
        ...
        setting.listObject[index] = this.GetComponent<Toggle>().isOn;
        uiManagement.SendOnChangeGameSettingObject(setting.listObject[index], index);
        uiManagement.SendSetting(this.gameObject.name, this.GetComponent<Toggle>().isOn);
    }

    public void UpdateToggle()
    {
        this.GetComponent<Toggle>().isOn = setting.listObject[index];
